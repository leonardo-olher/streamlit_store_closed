...
def container_radares(df, title, rgb, cat, dts_start, dts_end, hubs, hubs_media, key):
    ...
    with st.container():
        ...
        with g1:
            st.plotly_chart(radar(df, rgb=(rgb[0], rgb[1], rgb[2]), cat=cat, dt_start=dts_start[0], dt_end=dts_end[0], hubs=hubs, hubs_media=hubs_media[0], dts_min_max=(dts_start, dts_end)), config=config_radars(), key=f'{key}_radar_1')
            _, mid1, _ = st.columns([10,80,10])
            mid1.plotly_chart(bars(df, cat, hubs, dt_start=dts_start[0], dt_end=dts_end[0]), config={'displayModeBar': False}, key=f'{key}_bar_1', use_container_width=False)

        with g2:
            st.plotly_chart(radar(df, rgb=(rgb[0], rgb[1], rgb[2]), cat=cat, dt_start=dts_start[1], dt_end=dts_end[1], hubs=hubs, hubs_media=hubs_media[1], dts_min_max=(dts_start, dts_end)), config=config_radars(), key=f'{key}_radar_2')
            _, mid2, _ = st.columns([10,80,10])
            mid2.plotly_chart(bars(df, cat, hubs, dt_start=dts_start[1], dt_end=dts_end[1]), config={'displayModeBar': False}, key=f'{key}_bar_2', use_container_width=False)

        with g3:
            st.plotly_chart(radar(df, rgb=(rgb[0], rgb[1], rgb[2]), cat=cat, dt_start=dts_start[2], dt_end=dts_end[2], hubs=hubs, hubs_media=hubs_media[2], dts_min_max=(dts_start, dts_end)), config=config_radars(), key=f'{key}_radar_3')
            _, mid3, _ = st.columns([10,80,10])
            mid3.plotly_chart(bars(df, cat, hubs, dt_start=dts_start[2], dt_end=dts_end[2]), config={'displayModeBar': False}, key=f'{key}_bar_3', use_container_width=False)
