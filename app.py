...
if is_valid_user:
    ...
    @st.cache_data(ttl=3600) # ATUALIZAR BASE A CADA 1H
    def get_data():
        ...
        envs = {
            ...
            ,'SF_ACC': getenv('SF_ACC')
            ,'SLACK_ENDPOINT': getenv('SLACK_ENDPOINT')
            ,'SLACK_ENDPOINT_ERROR': getenv('SLACK_ENDPOINT_ERROR')
            ,'AIRFLOW_ENDPOINT': getenv('AIRFLOW_ENDPOINT')
            ,'TRAY_AUTH': getenv('TRAY_AUTH')
        }
        
        df = base_bucket(envs=envs)
