...
if is_valid_user:
    ...
    if need_update([6,0]): get_data.clear()

    df, updated_data = get_data()
    ...
    with st.container():
        ...
        with f1:
            ...
            start_date1 = today - timedelta(days=today.day-1) - relativedelta(months=2)
            end_date1 = start_date1 + relativedelta(months=1) - timedelta(days=1)
            ...
            if len(dt1) == 2:
                ...
                dt1_filtro = (start_date1, end_date1)

        with f2:
            today = current_date().date()
            start_date2 = today - timedelta(days=today.day-1) - relativedelta(months=1)
            end_date2 = start_date2 + relativedelta(months=1) - timedelta(days=1)
            ...
            if len(dt2) == 2:
                ...
                dt2_filtro = (start_date2, end_date2)

        with f3:
            today = current_date().date()
            start_date3 = today - timedelta(days=today.day-1)

            dt3 = st.date_input(
                'Data (Grafico 3)'
                ,(start_date3, today - timedelta(days=1))
                ,date(2025,2,1)
                ,today - timedelta(days=1)
                ,format="DD.MM.YYYY"
                ,key='data3')
    ...
    for cat in cat_filtro:
        container_radares(df, title, rgb=cat_color(cat), cat=cat, dts_start=(dt1_filtro[0], dt2_filtro[0], dt3_filtro[0]), dts_end=(dt1_filtro[1], dt2_filtro[1], dt3_filtro[1]), hubs=hubs_filtro, hubs_media=(hubs_media_dt1, hubs_media_dt2, hubs_media_dt3), key=cat)
